...
grad_p = [alpha * sp.diff(p, x_j) for x_j in x]
body_force = [ds - gp for (ds, gp) in zip(div_stress, grad_p)]

p_solid = labda * div_u - alpha * p

# Functions
u_lamb = sp.lambdify(x, u, "numpy")
r_lamb = sp.lambdify(x, r, "numpy")
p_slamb = sp.lambdify(x, p_solid, "numpy")
body_force_lamb = sp.lambdify(x, body_force, "numpy")
